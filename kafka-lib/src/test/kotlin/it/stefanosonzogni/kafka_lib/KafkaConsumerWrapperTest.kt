...
class KafkaConsumerWrapperTest {
    ...
    private fun retriesSource(): Stream<Arguments> {
        return Stream.of(
            Arguments.of(KafkaConsumerWrapper.Config(processingRetriesLimit = 0, processingRetriesDelayMs = 0), false),
            Arguments.of(KafkaConsumerWrapper.Config(processingRetriesLimit = 1, processingRetriesDelayMs = 0), true),
            Arguments.of(KafkaConsumerWrapper.Config(processingRetriesLimit = 2, processingRetriesDelayMs = 0), true),
            Arguments.of(KafkaConsumerWrapper.Config(processingRetriesLimit = 3, processingRetriesDelayMs = 0), true)
        )
    }

    @ParameterizedTest
    @MethodSource("retriesSource")
    fun `Consumer retries in case of failure`(config: KafkaConsumerWrapper.Config, expectsSuccess: Boolean) {
        val partition = 0
        var success = false
        val firstSampleRecord = ConsumerRecord(topic, partition, 100L, "key", "value")
        val processor = mockk<IKafkaProcessor<String, String>>()

        // process throws once and then passes
        coEvery { processor.process(firstSampleRecord) }
            .throws(Exception("some error"))
            .coAndThen { success = true }
        ...
        assertThat(success).isEqualTo(expectsSuccess)

        verify(exactly = 1) { consumerMock.commitSync(
            mapOf(TopicPartition(topic, partition) to OffsetAndMetadata(101L))
        ) }

        val calls = min(config.processingRetriesLimit + 1, 2)
        coVerify(exactly = calls) { processor.process(firstSampleRecord) }
